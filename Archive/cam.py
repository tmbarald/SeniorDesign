def cam():
    ...
    while (cap.isOpened() & out.isOpened()):
        ...
        retval, corners = cv.findChessboardCorners(frame, size)         # look for checkerboard pattern
        if(retval):
            foundCheckerboard = cv.drawChessboardCorners(frame, size, corners, retval)
            cv.imshow('Checkerboard', foundCheckerboard)   
